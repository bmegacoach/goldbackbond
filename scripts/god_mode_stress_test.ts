...
async function main() {
    console.log("--- Starting GOD-MODE Stress Test (Hardhat ESM Native) ---");
    
    // Explicitly destructure provider from ethers
    ...
    for (let slot = 0; slot <= 5; slot++) {
        ...
        let targetBalance = ethers.toBeHex(ethers.parseUnits("1M000000", 18).toString()); // 1 million
        targetBalance = ethers.zeroPadValue(ethers.toBeHex(ethers.parseUnits("1000000", 18)), 32);
